...
"""
import csv, html as html_module, io, json, os, time
...
st.set_page_config(
    page_title="TalentScan · High-Level Review",
    page_icon="📋",
    layout="wide",
...
def suitable_dept(r):
    """
    Returns the best-fit department based ONLY on CV evidence.
    Priority order:
      1. Use no_jd_suggested_depts if present (computed from CV keyword overlap with org taxonomy)
      2. Use no_jd_best_cluster if present
      3. Fall back to CV keyword scan against org taxonomy clusters
      4. "General" only if no evidence found
    Never infers or guesses — all results traceable to CV content.
    """
    # 1. Use system-computed dept match (from no_jd_agent org taxonomy scan)
    suggested = r.get("no_jd_suggested_depts") or []
    if isinstance(suggested, list) and suggested:
        top = suggested[0]
        if isinstance(top, dict):
            return top.get("cluster") or top.get("department") or "General"
    # 2. Use best_cluster from score_breakdown
    sb = r.get("score_breakdown") or {}
    if isinstance(sb, str):
        try: sb = json.loads(sb)
        except: sb = {}
    best = sb.get("best_cluster") or r.get("no_jd_best_cluster")
    if best:
        return best
    # 3. Evidence-only keyword scan on actual parsed CV fields (no role-title guessing)
    parsed = get_parsed(r)
    cv_fields = []
    for field in ["technical_skills","soft_skills","domain_expertise","certifications"]:
        cv_fields.extend(parse_list(parsed.get(field)))
    for exp in (parsed.get("experience") or []):
        cv_fields.append(str(exp.get("role") or ""))
        cv_fields.append(str(exp.get("description") or ""))
    for edu in (parsed.get("education") or []):
        cv_fields.append(str(edu.get("field") or ""))
        cv_fields.append(str(edu.get("degree") or ""))
    cv_blob = " ".join(cv_fields).lower()
    # FIX-2: Corrected org taxonomy keyword clusters
    # Changes: expanded HR cluster (emiratization/emiratisation + MOHRE/NAFIS),
    #          expanded Supply Chain cluster (logistics management degree title),
    #          narrowed Marketing cluster (removed generic 'communications').
    CLUSTERS = [
        (["mechanical engineering","electrical engineering","systems engineering","autocad","catia","avionics","aerodynamics"], "Engineering & Design"),
        (["missiles","weapons","defence","munitions","radar","guided systems","warhead"], "Missiles & Defence R&D"),
        (["flight test","prototype","telemetry","flight dynamics","airworthiness testing"], "Flight Test & Prototypes"),
        (["manufacturing","production","machining","fabrication","cnc","lean","assembly"], "Manufacturing & Production"),
        (["software","python","java","cloud","azure","cybersecurity","devops","database","erp","ai","machine learning","digital transformation","it systems"], "Digital & IT"),
        (["project management","program management","pmp","agile","scrum","pmo","stakeholder management"], "Program & Project Management"),
        (["quality management","iso 9001","as9100","safety management","qhse","audit","compliance","quality assurance"], "Quality, Safety & Compliance"),
        # FIX-2: 'logistics management' added to match degree title exactly
        (["supply chain","logistics management","logistics operations","procurement","purchasing","warehouse","inventory","vendor","transportation coordination"], "Supply Chain & Procurement"),
        (["finance","accounting","ifrs","gaap","tax","treasury","budgeting","financial reporting","accounts"], "Finance & Accounts"),
        # FIX-2: Added UAE-specific HR terms; both 'emiratization' and 'emiratisation' spellings
        (["human resources","hr management","recruitment","talent acquisition","learning and development","payroll",
          "emiratisation","emiratization","mohre","nafis","onboarding","employee relations","workforce"], "People & Culture / HR"),
        # FIX-2: Narrowed to explicit marketing terms — removed generic 'communications'
        (["marketing campaign","brand management","brand development","social media marketing","public relations","content marketing","digital marketing","advertising"], "Marketing & Communications"),
        (["business development","sales","commercial","contracts","government contracts","tenders","mod"], "Business Development & Sales"),
        (["facilities management","maintenance","hvac","construction","infrastructure","hard services"], "Facilities & Infrastructure"),
        (["legal","corporate law","contract law","regulatory affairs"], "Legal & Administration"),
        (["uav","drone","unmanned","uas","rpas","counter-uav","autonomous systems"], "UAV & Special Operations"),
        # FIX-2: New cluster for Government Relations / Public Affairs profiles
        (["government relations","public affairs","regulatory compliance","stakeholder engagement",
          "government communication","government policy","institutional relations","tamm","tasheel","icp","gdrfa"], "Government Relations & Public Affairs"),
    ]
    best_cluster, best_hits = "General", 0
    for keywords, cluster in CLUSTERS:
        hits = sum(1 for kw in keywords if kw in cv_blob)
        if hits > best_hits:
            best_hits = hits
...
def results_to_rows(results):
    ...
    for i, r in enumerate(results, 1):
        ...
        degree_raw = plain(r.get("degree") or (edu_list[0].get("degree") if edu_list else ""))
        # FIX-3: Append (In Progress) when education status indicates incomplete
        degree_status = (edu_list[0].get("status") or "") if edu_list else ""
        degree = (degree_raw + " (In Progress)") if degree_status == "in_progress" else degree_raw
        ...
        # FIX-5: Extended Skills Match fallback chain — never produce blank
        skills_match_items = (
            strengths
            or parse_list(r.get("transferable_skills"))
            or tech
            or parse_list(parsed.get("certifications"))
            or parse_list(parsed.get("domain_expertise"))
            or []
        )
        rows.append({
            "No":               str(i),
            ...
            "Skills Match":     "; ".join(skills_match_items[:10]),
            "Gaps":             "; ".join(gaps[:10]),
        })
    return rows
...
if not all_results:
    ...
    st.stop()
def build_pdf(results, role_name=""):
    """
    Generates a single PDF report containing ALL candidates in a table,
    matching exactly the columns shown on the High-Level Review page.
    """
    buffer = io.BytesIO()
    ...
    AMBER = colors.HexColor("#B35900")
    RED   = colors.HexColor("#C62828")
    LGRAY = colors.HexColor("#F7F8FC")
    MGRAY = colors.HexColor("#E2E6EA")
    GRAY  = colors.HexColor("#8A9BB0")

    small_style = ParagraphStyle("small", parent=styles["Normal"], fontSize=6, leading=8)
    ...
    # ── Header banner ─────────────────────────────────────────────────────────
    header_data = [[
        Paragraph("TalentScan AI — Candidate Review Report", title_style),
        Paragraph(
            f"Role: {role_name}  |  Exported: {datetime.now().strftime('%d %b %Y, %H:%M')}  |  {len(results)} candidates",
            sub_style,
        ),
    ...
    header_table = Table(header_data, colWidths=[120 * mm, 61 * mm])
    ...
    moderate = sum(1 for r in results if 0.45 <= r.get("composite_score", 0) < 0.65)
    weak = sum(1 for r in results if r.get("composite_score", 0) < 0.45)
    avg = (sum(r.get("composite_score", 0) for r in results) / total) if total else 0

    metric_data = [[
        Paragraph(f"<b>{total}</b><br/><font size=6 color='grey'>Total CVs</font>", normal_style),
        Paragraph(f"<b>{strong}</b><br/><font size=6 color='grey'>Strong Fit</font>", normal_style),
        Paragraph(f"<b>{moderate}</b><br/><font size=6 color='grey'>Moderate Fit</font>", normal_style),
        Paragraph(f"<b>{weak}</b><br/><font size=6 color='grey'>Weak Fit</font>", normal_style),
        Paragraph(f"<b>{avg:.0%}</b><br/><font size=6 color='grey'>Avg AI Score</font>", normal_style),
    ]]

    metric_table = Table(metric_data, colWidths=[36.2 * mm] * 5)
    ...
    # ── Candidate table ───────────────────────────────────────────────────────
    COL_WIDTHS = [7*mm, 32*mm, 18*mm, 10*mm, 8*mm, 22*mm, 26*mm, 20*mm, 12*mm, 16*mm, 30*mm]

    headers = [
        Paragraph("#", hdr_style),
        Paragraph("Candidate", hdr_style),
        Paragraph("Location", hdr_style),
        Paragraph("Gender", hdr_style),
        Paragraph("Exp", hdr_style),
        Paragraph("Degree/Major", hdr_style),
        Paragraph("Role/Employer", hdr_style),
        Paragraph("Dept", hdr_style),
        Paragraph("AI Score", hdr_style),
        Paragraph("Fit/Priority", hdr_style),
        Paragraph("AI Rationale", hdr_style),
    ]
    ...
    for rank, r in enumerate(results, 1):
        ...
        name = plain(r.get("full_name") or r.get("original_filename") or "Unknown")
        email = plain(r.get("email") or "—")
        phone = plain(r.get("phone") or get_parsed(r).get("phone") or "—")
        loc = plain(r.get("location") or r.get("city") or "—")
        gender = plain(r.get("gender") or "—")
        exp_yrs = plain(r.get("years_of_experience") or get_parsed(r).get("total_years_exp") or "—")

        parsed = get_parsed(r)
        edu = parsed.get("education") or []
        edu0 = edu[0] if edu and isinstance(edu[0], dict) else {}

        degree = plain(r.get("degree") or edu0.get("degree") or "—")
        major = plain(r.get("major") or edu0.get("field") or "—")

        exp_list = parsed.get("experience") or []
        employer = plain(r.get("current_employer") or (exp_list[0].get("company") if exp_list else "") or "—")
        desig = plain(r.get("current_designation") or (exp_list[0].get("role") if exp_list else "") or "—")

        dept = suitable_dept(r)
        fit_text, fit_cls = fit_label(comp)
        priority = priority_label(comp)
        rationale = plain(r.get("llm_rationale") or "—")

        score_pct = int(comp * 100)

        if fit_cls == "strong":
            row_bg = colors.HexColor("#E6F9EE")
            fit_color = GREEN
        elif fit_cls == "moderate":
            row_bg = colors.HexColor("#FFF4E5")
            fit_color = AMBER
        else:
            row_bg = colors.HexColor("#FFECEC")
            fit_color = RED

        sc_color = GREEN if comp >= 0.65 else (AMBER if comp >= 0.45 else RED)

        sc_hex = sc_color.hexval()[2:]
        fit_hex = fit_color.hexval()[2:]

        row_data = [
            Paragraph(f"<b>#{rank}</b>", normal_style),
            Paragraph(
                f"<b>{name}</b><br/>"
                f"<font size=6 color='grey'>{email}</font><br/>"
                f"<font size=5 color='#BCC0C4'>{phone}</font>",
                normal_style,
            ),
            Paragraph(loc, small_style),
            Paragraph(gender, small_style),
            Paragraph(f"<b>{exp_yrs}</b>", small_style),
            Paragraph(f"<b>{degree}</b><br/><font size=5 color='grey'>{major}</font>", small_style),
            Paragraph(f"<b>{desig}</b><br/><font size=5 color='grey'>{employer}</font>", small_style),
            Paragraph(dept, small_style),
            Paragraph(
                f"<b><font color='#{sc_hex}'>{score_pct}%</font></b><br/>"
                f"<font size=5 color='grey'>R:{int(rel*100)} S:{int(sem*100)} P:{int(pot*100)}</font>",
                small_style,
            ),
            Paragraph(
                f"<b><font color='#{fit_hex}'>{fit_text}</font></b><br/>"
                f"<font size=5 color='grey'>{priority}</font>",
                small_style,
            ),
            Paragraph(rationale[:220] + ("…" if len(rationale) > 220 else ""), small_style),
        ]

        table_data.append(row_data)
        row_bgs.append((len(table_data) - 1, row_bg))
    ...
    cand_table.setStyle(TableStyle(style_cmds))
    ...
    elements.append(
        Paragraph(
            f"Generated by TalentScan AI  |  {datetime.now().strftime('%d %b %Y, %H:%M')}  |  Confidential",
            footer_style,
        )
    )
...
def matches(r):
    ...
    edu0     = edu[0] if edu and isinstance(edu[0], dict) else {}
    degree_v = (r.get("degree") or edu0.get("degree", "")).lower()
    major_v  = (r.get("major")  or edu0.get("field",  "")).lower()
...
if "Top 5" in view_mode:
    st.markdown(
        ...
        padding:8px 14px;margin-bottom:1rem;font-size:0.8rem;
        color:#B35900;font-weight:600;">
        🏆 Showing Top 5 candidates by AI Score
        </div>
        """,
        unsafe_allow_html=True
    )
...
<div class="result-meta">
  <span>
    Showing <strong>{len(display_results)}</strong> candidate{"s" if len(display_results)!=1 else ""}
...
e1, e2, e3, e4 = st.columns([2, 2, 2, 4])
with e1:
    st.download_button( "Download CSV",
        data=build_csv(display_results),
        file_name=f"TalentScan_{_sr}_{_ts}.csv",
        mime="text/csv",
        use_container_width=True,
    )
with e2:
    st.download_button("Download Excel",
        data=build_excel(display_results, role_name=selected_label),
        file_name=f"TalentScan_{_sr}_{_ts}.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        use_container_width=True,
    )
with e3:
    # PDF always exports ALL filtered candidates (sorted_r), not just the current view (Top 5 etc.)
    st.download_button(
        "  Download PDF Report",
        data=build_pdf(sorted_r, role_name=selected_label),
        file_name=f"TalentScan_{_sr}_{_ts}.pdf",
        mime="application/pdf",
        use_container_width=True,
    )
with e4:
    st.markdown(
        f'<div style="font-size:0.72rem;color:#8A9BB0;padding-top:10px;">'
        f'CSV/Excel: <strong style="color:#1A1A2E;">{len(display_results)}</strong> candidate(s) shown  '
        f'&nbsp;|&nbsp;  PDF: <strong style="color:#1A1A2E;">{len(sorted_r)}</strong> all filtered candidate(s)  '
        f'for <strong style="color:#1A1A2E;">{esc(selected_label)}</strong></div>',
        unsafe_allow_html=True,
    )
st.markdown('<div class="divider"></div>', unsafe_allow_html=True)
# ── Table ─────────────────────────────────────────────────────────────────────
